...
renamed_casted AS (
SELECT
    {{ dbt_utils.generate_surrogate_key(['product_id', 'quantity', 'month']) }} AS budget_id, -- clave surrogate para presupuesto
    ...
    CAST(month AS DATE) AS month,                                                      -- mes en formato fecha, con conversión a UTC
